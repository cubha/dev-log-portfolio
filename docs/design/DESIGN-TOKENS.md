...
> **미정의 시맨틱 상태색 (신설 후보)**: `GuestbookListClient.tsx`가 비밀글 잠금에 `#F59E0B`(amber),
> 좋아요 활성에 `#F43F5E`(rose)를 하드코딩하고 있다. 브랜드색도 드리프트도 아닌 **토큰이 없는 상태색**이다.
> `--color-success`/`--color-error`를 승격한 전례와 동일한 절차로 신설하는 것이 정합적이다.
